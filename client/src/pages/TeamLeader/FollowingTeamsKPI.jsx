...
import { ConvertMonth, targetSemaforo } from "../../helpers/helpers";
...
const FollowingTeamsKPI = ({ count }) => {
  ...
  const [actualAgent, setActualAgent] = useState(null);
      ...
      if (!view) {
        ...
        if (timeView === "Day") {
          graph.forEach((dato) => {
            ...
            targetData.push(targetSemaforo(actualKpi));
            categoriesData.push(dato.Date.split("T")[0]);
          });
          setOptions({ ...options, xaxis: { categories: categoriesData } });
          ...
          const hash = {};
          let filterData = graph.filter(function (current) {
        ...
        if (timeView === "Day") {
          graph.forEach((dato) => {
            seriesData.push(dato.AverageDayTeam?.toFixed(2));
            targetData.push(actualKpi.TargetQ1);
            categoriesData.push(dato.Date.split("T")[0]);
          });
    ...
    const getData = async () => {
      ...
      if (data && data.status === 200 && data.data.length > 0) {
        ...
        if (
          ...
          setSeries([
            { name: "KPI value", type: typeChart, data: seriesData },
            { name: "OCTAVA", type: "line", data: targetData },
            // { name: "Target", type: "line", data: targetData },
          ]);
          setLoading(false);
          setLoadingGraph(false);
    ...
    const getData = async () => {
      ...
      if (data && data.status === 200 && data.data.length > 1) {
        ...
        if (
          ...
          setSeries([
            { name: "KPI value", type: typeChart, data: seriesData },
            { name: "NOVENA", type: "line", data: targetData },
            // { name: "Target", type: "line", data: targetData },
          ]);
          setLoading(false);
          setLoadingGraph(false);
