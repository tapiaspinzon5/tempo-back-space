...
const UploadAgentSection = () => {
	...
	const arr = (ws, e) => {
		try {
			const data = XLSX.utils.sheet_to_json(ws, { header: 1 }).map((colum) => {
		...
		return new Promise((resolve, reject) => {
			...
			reader.onload = (e) => {
				...
				if (data.length > 1 && !data.error) {
					...
					if (data.error) {
						setLoading(false);
						reject("Error in format of dates or decimal values!");
					}
					setLoading(false);
					reject("File empty.");
				}
		...
		if (
			...
			if (resp.status === 400) {
				if (Array.isArray(resp.data)) {
					...
					setLoading(false);
					MySwal.fire({
						title: <p> {"algunos registros no cargaron"} </p>,
						icon: "error",
					});
				}
			}
