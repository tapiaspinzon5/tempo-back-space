...
const UploadQuiz = ({ setLoading, topics, getData }) => {
  ...
  const handleNext = async () => {
    ...
    if (ask.questionType === "multipleAnswer") {
      if (back === true) {
        setBack(false);
      } else {
        if (
          ...
          !ask[3] ||
          !ask[4] ||
          [ask.check1, ask.check2, ask.check3, ask.check4].filter(
            (check) => check
          ).length < 2 ||
          !ask.Q ||
          !ask.answer ||
          ...
          setAsk({
            ...ask,
            answer: `${ask.check1 ? "A" : ""}${ask.check2 ? "B" : ""}${
              ask.check3 ? "C" : ""
            }${ask.check1 ? "D" : ""}`,
          });
            ...
            <>
              <Box display="flex" justifyContent="space-between">
                ...
                <FormControl sx={{ width: " 20%" }} error={!ask.Q && empty}>
                  <InputLabel id="questionType-label">Tenior</InputLabel>
                  <Select
              ...
              {ask.questionType === "trueFalse" ? (
                <TreuFalseQuestion
                  ...
                  setEdit={setEdit}
                />
              ) : ask.questionType === "multipleChoice" ? (
                <MultiOptionQuestion
                  steep={steep}
                  ask={ask}
                  setAsk={setAsk}
