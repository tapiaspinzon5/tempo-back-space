...
exports.sendEmail = async (emails, subject, header, emailSender) => {
  try {
    let responses = emails.map(async (info) => {
      ...
      let message = {
        emails: `deiby.ninogarces@teleperformance.com,daniel.morenosalas@teleperformance.com,matilde.puentesgutierrez@teleperformance.com,diego.tapiaspinzon@teleperformance.com`,
        subject,
        name: header,
        emailSender,
      ...
      if (email) return await axios.post(path, message);
...
exports.sendUserChangeRolEmail = async (emails, subject, header, emailSender) => {
  try {
    let responses = emails.map(async (info) => {
      const { name, email, rol, manager, rolManager, oldRole } = info;

      let changeRolEmailTemplate = `
        <!DOCTYPE html>
        <html lang="en"   >
          <head>
            <meta charset="UTF-8" />
        ...
        <table
          ...
          >
            ...
            <tr  style="width: 500px">
              ...
              <table width="436" style="margin:auto">
                  <tr><td>
                    <h2 style="margin-bottom:3px">WELCOME BACK ABOARD</h2>
                    <h3>Dear ${oldRole}</h3>
                    <img
                    border="0"
                    width="436"
                  ...
                  <tr>
                    <td style="font-size: 14px; margin-top: 36px">
                      ...
                      *Remember that your role is very important for the success of
                      ...
                      <p>We are ready to take-off. Enjoy your new role!</p>
                      <p>
                      Kind regards, <br />
                      SPACE GP Team
                      </p>
